...
async function archiveGitTree(repPath, tree, archive) {
  ...
  const process = async ({ type, oid, path }) => {
    if (type === 'tree' || type === 'commit') {
      ...
      archive.append(null, { name: `${path}/`, date: new Date('1980-01-01T00:00:00') });
    } else {
      // blob
      const stream = await createBlobReadStream(repPath, oid);
      archive.append(stream, { name: path, date: new Date('1980-01-01T00:00:00') });
    }
  };
...
async function collectFSEntries(dirPath, allEntries) {
  ...
  const ignore = Ignore({ignorecase: false});
  ...
  const process = (entry, cb) => {
    ...
    if (p.length) {
      if (entry.stats.isDirectory()) {
        archive.append(null, { name: `${p}/`, date: new Date('1980-01-01T00:00:00') });
      } else {
        archive.append(fse.createReadStream(entry.path), { name: p, date: new Date('1980-01-01T00:00:00') });
      }
    }
    cb();
  };
